...
const ToolCard = ({ title, excerpt, external_link, category, image_url, slug }) => {
  return (
    <div className="bg-white rounded-xl shadow-md overflow-hidden transition-all duration-300 hover:shadow-xl flex flex-col h-full">
      ...
      <div className="p-5 flex-1 flex flex-col">
        ...
        <span className="inline-block px-3 py-1 text-xs font-medium bg-blue-100 text-blue-600 rounded-full mb-2">
          ...
          <Link href={external_link} target="_blank" rel="noopener noreferrer" className="text-blue-600 hover:text-blue-800 text-sm font-medium font-sen transition-colors">
            View Tool →
          </Link>
        </div>
      </div>
